...
/*    @Test
    public void testAssignContainerPreCheck() {
      AppSchedulable app = mock(AppSchedulable.class);
      Mockito.when(app.getResourceUsage()).thenReturn(maxResource);
      schedulable.addAppSchedulable(app);
      RMNode fakeNode = MockNodes.newNodeInfo(1, maxResource, 1, "127.0.0.1");
       boolean couldAssignMoreContainer = schedulable.assignContainerPreCheck(new FSSchedulerNode(fakeNode, true));
      assertFalse("ResourceUsage is greater than max allowed ", couldAssignMoreContainer);
   }
  */
